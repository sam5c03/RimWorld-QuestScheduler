...
namespace QuestScheduler
{
    ...
    public class QuestSchedulerMod : Mod
    {
        ...
        public static void GenerateQuestFinal(int tileIndex)
        {
            ...
            // 【新增修改：修復 Grammar unresolvable 錯誤】
            // 為任務文本提供一個預設的「發現方式」，防止語法解析失敗
            ...
            {
                // 【保留】原有的安全機制：確保簡單任務絕對不會套用自訂生成攔截
                CustomRaidGenerator.isSpawningCustomQuest = false;
            }

            // 下面的 try 區塊完全不用動，保持你原本的代碼即可
            try
            {
                ...
                if (quest != null)
                {
                    ...
                    if (defNameLow.Contains("site") || defNameLow.Contains("camp") || defNameLow.Contains("outpost"))
                    {
                        if (quest.State == QuestState.NotYetAccepted)
                        {
                            ...
                            // 【新增】將生成的據點座標加入記憶清單
                            var sitePart = quest.PartsListForReading.OfType<QuestPart_SpawnWorldObject>().FirstOrDefault();
                            if (sitePart != null && sitePart.worldObject != null)
                            {
                                if (!settings.customSiteTiles.Contains(sitePart.worldObject.Tile))
                                    settings.customSiteTiles.Add(sitePart.worldObject.Tile);
                            }
    ...
    {
        // 智能判斷：現在是不是在我們標記的自製任務地圖上生成小人？
        // 智能判斷：現在是不是在我們標記的自製任務地圖上生成小人？
        public static bool IsCustomSiteGeneration()
        {
            // 如果是剛按下按鈕生成的瞬發任務，全域變數會是 true
            if (CustomRaidGenerator.isSpawningCustomQuest) return true;

            // 【RimWorld 1.6 API 確認】
            // 在 1.6 的 Assembly-CSharp.dll 中，沒有 IsGeneratingMap。
            // 正確的官方 API 是直接讀取 mapBeingGenerated 變數。
            Map currentMap = Verse.MapGenerator.mapBeingGenerated;
            if (currentMap != null)
            {
                // 檢查當前正在生成的地塊，是否屬於我們記錄過的自製任務地塊
                return QuestSchedulerMod.settings.customSiteTiles.Contains(currentMap.Tile);
            }
            return false;
        }
        ...
        {
            if (IsCustomSiteGeneration())
            {
                if (!request.FixedBiologicalAge.HasValue) request.FixedBiologicalAge = Rand.Range(QuestSchedulerMod.settings.customQuestAgeMin, QuestSchedulerMod.settings.customQuestAgeMax);
                request.AllowedDevelopmentalStages = DevelopmentalStage.Adult;
                if (QuestSchedulerMod.settings.customQuestXenotype != null && request.ForcedXenotype == null)
                    request.ForcedXenotype = QuestSchedulerMod.settings.customQuestXenotype;
                if (!request.FixedGender.HasValue) request.FixedGender = Rand.Value < (QuestSchedulerMod.settings.customQuestFemaleRatio / 100f) ? Gender.Female : Gender.Male;
            }
        ...
        {
            if (IsCustomSiteGeneration() && __result?.story != null)
            {
    ...
    public static class Patch_ForcePawnCount
    {
        ...
        public static void Postfix(ref IEnumerable<Pawn> __result, PawnGroupMakerParms parms)
        {
            ...
            {
                // 只針對敵對派系進行人數覆寫，以免把地圖上的中立商人或古老威脅也複製了
                if (parms.faction != null && parms.faction.HostileTo(Faction.OfPlayer))
                {
                    List<Pawn> originalPawns = __result.ToList();
                    int targetCount = QuestSchedulerMod.settings.customQuestPawnCount;

                    if (originalPawns.Count == 0) return; // 防呆機制

                    if (originalPawns.Count > targetCount)
                    {
                        // 點數太多導致生成過多：強制刪除多餘的敵人
                        for (int i = targetCount; i < originalPawns.Count; i++)
                            Find.WorldPawns.PassToWorld(originalPawns[i], PawnDiscardDecideMode.Discard);
                        __result = originalPawns.Take(targetCount).ToList();
                    }
                    else if (originalPawns.Count < targetCount)
                    {
                        // 點數太少導致敵人不夠：強制無限複製生成的敵人直到湊滿
                        List<Pawn> newPawns = new List<Pawn>(originalPawns);
                        int needed = targetCount - originalPawns.Count;

                        for (int i = 0; i < needed; i++)
                        {
                            var template = originalPawns.RandomElement(); // 隨機挑選剛生成的一名敵人作為模板
                            PawnGenerationRequest req = new PawnGenerationRequest(template.kindDef, parms.faction);
                            Pawn p = PawnGenerator.GeneratePawn(req); // 生成新敵人 (會自動觸發上方的人種/年齡攔截)
                            newPawns.Add(p);
                        }
                        __result = newPawns;
                    }
