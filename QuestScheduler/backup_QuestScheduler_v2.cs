...
namespace QuestScheduler
{
    ...
    public class QuestSchedulerMod : Mod
    {
        ...
        private void DrawCustomQuestTab(Rect rect)
        {
            ...
            Rect viewRect = new Rect(0, 0, outRect.width - 20f, 850f);
            ...
            if (isComplex)
            {
                ...
                string facLabel = customQuestFaction != null ? $"{customQuestFaction.Name} [{customQuestFaction.PlayerRelationKind.GetLabel()}]" : "系統預設 (推薦)";
                ...
                listing.CheckboxLabeled("7. 🎁 啟用自訂任務獎勵", ref settings.useCustomRewards, "開啟後將替換系統獎品。若任務不支援會彈出警告，請關閉後再生成。");
            ...
            else
            {
                // 如果是簡單任務，自動隱藏進階參數
                listing.Gap(20f);
                listing.Label("<color=grey>💡 此任務為事件型或簡單任務 (如法令、野生動物、流浪者等)。\n進階自訂參數已自動隱藏，系統將以原版預設設定為您安全生成。</color>");
                listing.Gap(20f);
            }
        ...
        public static void GenerateQuestFinal(int tileIndex)
        {
            ...
            // 若為複雜任務，才覆寫 PawnGenerator 變數並注入點數/派系
            if (isComplex)
            {
                    ...
                    if (isComplex && settings.useCustomRewards && settings.customRewards.Count > 0)
                    {
                        ...
                        if (choicePart == null || endSuccessPart == null)
                        {
                            ...
                            Find.WindowStack.Add(new Dialog_MessageBox(
                                "⚠️ 偵測到自訂獎勵與系統衝突！\n\n您選擇的任務不支援標準的獎勵選擇結構，強制替換會導致系統崩潰。\n\n請在設定中【關閉自訂任務獎勵】以使用系統預設獎品池，或選擇其他類型的任務。",
                                "了解並返回"));
                            return;
                        }
