...
public static void GenerateQuestFinal(int tileIndex)
        {
            ...
            if (isComplex)
            {
                ...
                // 【Log 記錄 1：確認生成任務時的參數】
                Log.Message($"[QuestScheduler] 正在生成任務: {qDef.defName}, 預期人數: {settings.customQuestPawnCount}, 預期人種: {(settings.customQuestXenotype?.label ?? "預設")}");
            }
            else
            {
            ...
            try
            {
                ...
                if (quest != null)
                {
                    ...
                    // 無論手動還是隨機，都把最終的地塊位置加入記憶
                    foreach (var part in quest.PartsListForReading.OfType<QuestPart_SpawnWorldObject>())
                    {
                        if (part.worldObject != null)
                        {
                            int actualTile = part.worldObject.Tile;
                            if (actualTile >= 0 && !settings.customSiteTiles.Contains(actualTile))
                            {
                                settings.customSiteTiles.Add(actualTile);
                            }
                            // 【Log 記錄 2：確認最終地塊】
                            Log.Message($"[QuestScheduler] 據點已生成並記憶！真實地圖座標: {actualTile} ({(tileIndex >= 0 ? "手動" : "隨機")})");
                        }
                    }

                    // 【還原】與 V9 備份完全一致的自訂獎勵邏輯
        ...
        public static void Postfix(ref IEnumerable<Pawn> __result, PawnGroupMakerParms parms)
        {
            if (parms.tile >= 0 && QuestSchedulerMod.settings.customSiteTiles.Contains(parms.tile) && QuestSchedulerMod.settings.customQuestPawnCount > 0)
            {
                if (parms.faction != null && parms.faction.HostileTo(Faction.OfPlayer))
                {
                    ...
                    // 【Log 記錄 3：進場時系統原始生成的數量】
                    Log.Message($"[QuestScheduler] 玩家部隊抵達！系統原本基於點數生成了 {vanillaPawns.Count} 名敵人。準備將其全部丟棄，並強制捏造 {targetCount} 名自訂敵人...");

                    // 1. 將系統預設生成的敵人 (例如那 3 個豬皮人) 全部從世界上刪除
                    ...
                    // 尋找「通用戰鬥兵種」作為白紙模板，徹底破解豬皮人等派系綁定
                    PawnKindDef genericKind = DefDatabase<PawnKindDef>.GetNamedSilentFail("Mercenary_Gunner") ??
                                              DefDatabase<PawnKindDef>.GetNamedSilentFail("Town_Guard") ??
                    ...
                    for (int i = 0; i < targetCount; i++)
                    {
                        PawnGenerationRequest req = new PawnGenerationRequest(
                            ...
                            FixedGender: Rand.Value < (QuestSchedulerMod.settings.customQuestFemaleRatio / 100f) ? Gender.Female : Gender.Male,
                            FixedBiologicalAge: Rand.Range(QuestSchedulerMod.settings.customQuestAgeMin, QuestSchedulerMod.settings.customQuestAgeMax)
                        );
                    ...
                    // 【Log 記錄 4：任務完成】
                    Log.Message($"[QuestScheduler] 替換成功！已生成 {newPawns.Count} 名全新敵人。");
                }
            }
        }
    }
